function BlowfishEngine(key) {
  const KP = [0x243F6A88, 0x85A308D3, 0x13198A2E,
  ...
  const KS0 = [0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB,
  ...
  const KS1 = [0x4B7A70E9, 0xB5B32944, 0xDB75092E,
  ...
  const KS2 = [0xE93D5A68, 0x948140F7, 0xF64C261C,
  ...
  const KS3 = [0x3A39CE37, 0xD3FAF5CF, 0xABC27737,
  ...
  const ROUNDS = 16;
  const BLOCK_SIZE = 8; // bytes = 64 bits
  const SBOX_SK = 256;
  const P_SZ = ROUNDS + 2;
  let S0 = Array(SBOX_SK);
  let S1 = Array(SBOX_SK);
  let S2 = Array(SBOX_SK);
  let S3 = Array(SBOX_SK);
  let P = Array(P_SZ);

  setKey(key);
  
  this.decrypt = function(packet) {
    const count = packet.length / BLOCK_SIZE;
    const outData = [];

    for (let i = 0; i < count; i++) {
      decryptBlock(packet, i * BLOCK_SIZE, outData, i * BLOCK_SIZE);
    }

    return outData;
  }

  this.encrypt = function(packet) {
    const count = packet.length / BLOCK_SIZE;
    const outData = [];

    for (let i = 0; i < count; i++) {
      encryptBlock(packet, i * BLOCK_SIZE, outData, i * BLOCK_SIZE);
    }
  ...
  function processTable(xl, xr, table) {
  ...
  function setKey(key) {	
    S0 = KS0.slice(0, SBOX_SK);
    S1 = KS1.slice(0, SBOX_SK);
    S2 = KS2.slice(0, SBOX_SK);
    S3 = KS3.slice(0, SBOX_SK);
    P = KP.slice(0, P_SZ);
  ...
  function encryptBlock(src, srcIndex, dst, dstIndex) {
  ...
  function decryptBlock(src, srcIndex, dst, dstIndex) {
    let xl = BytesTo32bits(src, srcIndex);
    let xr = BytesTo32bits(src, srcIndex + 4);
  ...
  function Bits32ToBytes(inData, b, offset) {
    b[offset] = inData & 255;
