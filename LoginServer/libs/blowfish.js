class BlowfishEngine {
  constructor(key) {
    this._KP = [0x243F6A88, 0x85A308D3, 0x13198A2E,
    ...
    this._KS0 = [0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB,
    ...
    this._KS1 = [0x4B7A70E9, 0xB5B32944, 0xDB75092E,
      ...
      0x37C2DADC, 0xC8B57634, 0x9AF3DDA7, 0xA9446146, 0x0FD0030E,
      ...
      0xA67BC883, 0xB17F37D1, 0x018CFF28, 0xC332DDEF, 0xBE6C5AA5,
      ...
      0x4C70A239, 0xD59E9E0B, 0xCBAADE14, 0xEECC86BC, 0x60622CA7,
    ...
    this._KS2 = [0xE93D5A68, 0x948140F7, 0xF64C261C,
    ...
    this._KS3 = [0x3A39CE37, 0xD3FAF5CF, 0xABC27737,
    ...
    this._ROUNDS = 16;
    this._BLOCK_SIZE = 8; // bytes = 64 bits
    this._SBOX_SK = 256;
    this._P_SZ = this._ROUNDS + 2;
    this._S0 = Array(this._SBOX_SK);
    this._S1 = Array(this._SBOX_SK);
    this._S2 = Array(this._SBOX_SK);
    this._S3 = Array(this._SBOX_SK);
    this._P = Array(this._P_SZ);
  
    this._setKey(key);
  }
  
  decrypt(packet) {
    const count = packet.length / this._BLOCK_SIZE;
    const outData = [];

    for (let i = 0; i < count; i++) {
      this._decryptBlock(packet, i * this._BLOCK_SIZE, outData, i * this._BLOCK_SIZE);
    }
  ...
  encrypt(packet) {
    ...
    for (let i = 0; i < count; i++) {
      this._encryptBlock(packet, i * this._BLOCK_SIZE, outData, i * this._BLOCK_SIZE);
    }
  ...
  _processTable(xl, xr, table) {
    const size = table.length;

    for (let s = 0; s < size; s += 2) {
      xl ^= this._P[0];

      for (let i = 1; i < this._ROUNDS; i += 2) {
        xr ^= this._F(xl) ^ this._P[i];
        xl ^= this._F(xr) ^ this._P[i + 1];
      }

      xr ^= this._P[this._ROUNDS + 1];
      table[s] = xr;
      table[s + 1] = xl;
      xr = xl;
      xl = table[s];
  ...
  _setKey(key) {	
    this._S0 = this._KS0.slice(0, this._SBOX_SK);
    this._S1 = this._KS1.slice(0, this._SBOX_SK);
    this._S2 = this._KS2.slice(0, this._SBOX_SK);
    this._S3 = this._KS3.slice(0, this._SBOX_SK);
    this._P = this._KP.slice(0, this._P_SZ);

    const keyLength = key.length;
    let keyIndex = 0;
    
    for (let i = 0; i < this._P_SZ; i++) {
      let data = 0;
    ...
    this._processTable(0, 0, this._P);
    this._processTable(this._P[this._P_SZ - 2], this._P[this._P_SZ - 1], this._S0);
    this._processTable(this._S0[this._SBOX_SK - 2], this._S0[this._SBOX_SK - 1], this._S1);
    this._processTable(this._S1[this._SBOX_SK - 2], this._S1[this._SBOX_SK - 1], this._S2);
    this._processTable(this._S2[this._SBOX_SK - 2], this._S2[this._SBOX_SK - 1], this._S3);
  }
  
  _encryptBlock(src, srcIndex, dst, dstIndex) {
    let xl = this._bytesTo32bits(src, srcIndex);
    let xr = this._bytesTo32bits(src, srcIndex + 4);
    
    xl ^= this._P[0];

    for (var i = 1; i < this._ROUNDS; i += 2) {
      xr ^= this._F(xl) ^ this._P[i];
      xl ^= this._F(xr) ^ this._P[i + 1];
    }

    xr ^= this._P[this._ROUNDS + 1];
    this._bits32ToBytes(xr, dst, dstIndex);
    this._bits32ToBytes(xl, dst, dstIndex + 4);
  }

  _decryptBlock(src, srcIndex, dst, dstIndex) {
    let xl = this._bytesTo32bits(src, srcIndex);
    let xr = this._bytesTo32bits(src, srcIndex + 4);

    xl ^= this._P[this._ROUNDS + 1];

    for (let i = this._ROUNDS; i > 0; i -= 2) {
      xr ^= this._F(xl) ^ this._P[i];
      xl ^= this._F(xr) ^ this._P[i - 1];
    }
  ...
  _bits32ToBytes(inData, b, offset) {
    b[offset] = inData & 255;
    b[offset + 1] = (inData >> 8) & 255;
    b[offset + 2] = (inData >> 16) & 255;
    b[offset + 3] = (inData >> 24) & 255;
  }
}

module.exports = BlowfishEngine;